...
function unpatchedBundleFixture() {
  return [
    ...
    "const CATALOG = [",
    modelBlock("claude-fable-5-1", "Claude Fable 5.1") + ",",
    ...
    '\tif (model !== "claude-fable-5-1" && model !== "claude-opus-5" && model !== "claude-sonnet-5") return "max";',
    "}",
...
function legacyFableBundleFixture() {
  return unpatchedBundleFixture()
    .replaceAll("claude-fable-5-1", "claude-fable-5")
    .replaceAll("Claude Fable 5.1", "Claude Fable 5");
}

function executePatchedFixture(input = unpatchedBundleFixture()) {
  const { bundle } = patchBundle(input);
...
test("Astra has a separate medium/272k hybrid model with scoped environment and effort", () => {
  const api = executePatchedFixture();
  const models = api.allModels.filter((m) => m.slug === "gpt-6-astra");
  assert.equal(models.length, 1);
  assert.equal(models[0].name, "GPT-6 Astra");
  const caps = api.getClaudeModelCapabilities("gpt-6-astra");
  assert.equal(
    caps.optionDescriptors.find((d) => d.id === "effort").options.find((o) => o.isDefault).value,
    "medium",
  );
  const selection = { model: "gpt-6-astra" };
  assert.equal(api.selectedClaudeContextWindow(selection), 272000);
  assert.equal(api.buildTextSettings(selection).autoCompactWindow, 240000);
  assert.equal(api.buildAgentSettings(selection).autoCompactWindow, 240000);
  const original = { OTHER_SETTING: "preserved", CLAUDE_CODE_MAX_CONTEXT_TOKENS: "260000" };
  assert.deepEqual(api.buildAgentEnvironment(selection, original), {
    ...original,
    CLAUDE_CODE_MAX_CONTEXT_TOKENS: "272000",
    CLAUDE_CODE_AUTO_COMPACT_WINDOW: "240000",
  });
  assert.equal(original.CLAUDE_CODE_MAX_CONTEXT_TOKENS, "260000");
  assert.equal(api.buildAgentEnvironment({ model: "unknown-model" }, original), original);
  assert.equal(
    api.buildAgentEnvironment({ model: "qwen3.8-27b" }, original).CLAUDE_CODE_MAX_CONTEXT_TOKENS,
    "131072",
  );
  assert.equal(api.resolveClaudeApiModelId(selection), "gpt-6-astra[effort=medium]");
  assert.equal(
    api.resolveClaudeApiModelId({
      model: "gpt-6-astra",
      options: [{ id: "effort", value: "max" }],
    }),
    "gpt-6-astra[effort=max]",
  );
  const patched = patchBundle(unpatchedBundleFixture()).bundle;
  assert.equal(patchBundle(patched).bundle, patched);
  assert.match(patched, /CLAUDE_CODE_MAX_CONTEXT_TOKENS: "272000"/);
});

test("Astra service tier defaults to standard and forwards explicit priority through the hybrid router", () => {
  const api = executePatchedFixture();
  const tier = api
    .getClaudeModelCapabilities("gpt-6-astra")
    .optionDescriptors.find((d) => d.id === "serviceTier");
  assert.ok(tier);
  assert.deepEqual(
    tier.options.map((o) => o.value),
    ["default", "priority"],
  );
  assert.equal(tier.options.find((o) => o.isDefault).value, "default");
  const { preparePayload } = require("./claude-hybrid-router.cjs");
  for (const value of ["default", "priority", "invalid"]) {
    const model = api.resolveClaudeApiModelId({
      model: "gpt-6-astra",
      options: [
    ...
    const result = preparePayload({ model, messages: [] });
    assert.equal(result.provider, "gpt");
    assert.equal(result.payload.model, "gpt-6-astra");
    assert.equal(result.payload.output_config.effort, "high");
    assert.equal(result.payload.speed, value === "priority" ? "fast" : undefined);
  }
});

test("compatible hybrid models share Astra context limits without changing Qwen or explicit 1M", () => {
  const api = executePatchedFixture();
  for (const model of [
    "claude-fable-5-1",
    "claude-opus-5",
    ...
    assert.equal(api.selectedClaudeContextWindow(selection), 272000);
    assert.equal(api.buildAgentSettings(selection).autoCompactWindow, 240000);
    assert.equal(api.buildAgentEnvironment(selection, {}).CLAUDE_CODE_MAX_CONTEXT_TOKENS, "272000");
    const extended = { model, options: [{ id: "contextWindow", value: "1m" }] };
    assert.equal(api.selectedClaudeContextWindow(extended), 1000000);
    assert.equal(api.buildAgentEnvironment(extended, {}).CLAUDE_CODE_MAX_CONTEXT_TOKENS, "1000000");
  }
  assert.equal(api.selectedClaudeContextWindow({ model: "qwen3.8-27b" }), 131072);
});

test("900k is opt-in across hybrid models and switching back restores 272k", () => {
  const api = executePatchedFixture();
  for (const model of [
    "gpt-6-astra",
    "claude-fable-5-1",
    ...
    const selection = { model, options: [{ id: "contextWindow", value: "900k" }] };
    const caps = api.getClaudeModelCapabilities(model);
    assert.ok(
      caps.optionDescriptors
        .find((d) => d.id === "contextWindow")
        .options.some((o) => o.value === "900k" && !o.isDefault),
    );
    assert.equal(api.selectedClaudeContextWindow(selection), 900000);
    assert.equal(api.buildTextSettings(selection).autoCompactWindow, 850000);
    assert.equal(api.buildAgentSettings(selection).autoCompactWindow, 850000);
    assert.equal(api.buildAgentEnvironment(selection, {}).CLAUDE_CODE_MAX_CONTEXT_TOKENS, "900000");
    assert.equal(
      api.buildAgentEnvironment(selection, {}).CLAUDE_CODE_AUTO_COMPACT_WINDOW,
      "850000",
    );
    assert.equal(api.selectedClaudeContextWindow({ model }), 272000);
    assert.equal(api.buildAgentSettings({ model }).autoCompactWindow, 240000);
  }
  assert.equal(
    api.selectedClaudeContextWindow({
      model: "qwen3.8-27b",
      options: [{ id: "contextWindow", value: "900k" }],
    }),
    131072,
  );
});
...
test("usage rate patch preserves the native v0.0.37 qualified-key pricing implementation", () => {
  ...
  assert.match(
    result.bundle,
    /\/\* t3-patch:canonical-usage-rate-priority \*\/[\r\n]+function parseRateTable/,
  );
...
test("patch exposes the approved four-model option profiles", () => {
  ...
  for (const [slug, profile] of Object.entries(expected)) {
    ...
    assert.equal(
      effort.options.find((option) => option.isDefault)?.value,
    ...
    assert.deepEqual(
      ...
      [
        ...
        { value: "900k", label: "900k (higher usage)" },
        { value: "1m", label: "1M" },
      ],
      slug,
    );
    assert.equal(runtimeContextWindow(slug, "default"), 272000, slug);
    assert.equal(runtimeContextWindow(slug, "1m"), 1000000, slug);
  }
});

test("patch upgrades a legacy Claude Fable 5 picker entry to Fable 5.1", () => {
  const { bundle, status } = patchBundle(legacyFableBundleFixture());
  assert.equal(status, "patched");
  assert.match(bundle, /slug: "claude-fable-5-1"/);
  assert.match(bundle, /name: "Claude Fable 5\.1"/);
  assert.doesNotMatch(bundle, /slug: "claude-fable-5"[,\r\n]/);
});

test("patch upgrades an already-patched legacy Fable context model list", () => {
  const legacyPatched = patchBundle(unpatchedBundleFixture())
    .bundle.replaceAll("claude-fable-5-1", "claude-fable-5")
    .replaceAll("Claude Fable 5.1", "Claude Fable 5");
  const { bundle, status } = patchBundle(legacyPatched);
  assert.equal(status, "patched");
  assert.match(bundle, /HYBRID_CONTEXT_MODELS = new Set\(\["claude-fable-5-1"/);
  assert.doesNotMatch(bundle, /HYBRID_CONTEXT_MODELS = new Set\(\["claude-fable-5"[,\]]/);
});

test("bypass permissions omits the redundant canUseTool stdio bridge", () => {
  const { bundle } = patchBundle(unpatchedBundleFixture());
  assert.match(bundle, /\.\.\.permissionMode === "bypassPermissions" \? \{\} : \{ canUseTool \},/);
  assert.doesNotMatch(
    bundle,
    /includePartialMessages: true,\r?\n\s+canUseTool,\r?\n\s+supportedDialogKinds:/,
  );
});

test("permission bridge remains available outside bypass mode", () => {
  for (const mode of ["bypassPermissions", "default", "acceptEdits", "plan"]) {
    const input = unpatchedBundleFixture()
      .replace(
        'const permissionMode = "bypassPermissions";',
        `const permissionMode = ${JSON.stringify(mode)};`,
      )
      .replace("return queryOptions.env;", "return queryOptions;");
    const options = executePatchedFixture(input).buildAgentEnvironment(
      { model: "gpt-6-astra" },
      {},
    );
    assert.equal(Object.hasOwn(options, "canUseTool"), mode !== "bypassPermissions", mode);
    if (mode !== "bypassPermissions") assert.equal(typeof options.canUseTool, "function");
    assert.equal(options.env.CLAUDE_CODE_MAX_CONTEXT_TOKENS, "272000");
  }
});

test("all patch stages accept CRLF and remain byte-idempotent", () => {
  const input = unpatchedBundleFixture().replaceAll("\n", "\r\n");
  const first = patchBundle(input);
  const second = patchBundle(first.bundle);
  assert.equal(second.status, "already-patched");
  assert.equal(second.bundle, first.bundle);
  assert.equal(
    executePatchedFixture(input).selectedClaudeContextWindow({ model: "gpt-6-astra" }),
    272000,
  );
});

test("upgrading the old Sol high default preserves every extended-context stage", () => {
  const patched = patchBundle(unpatchedBundleFixture()).bundle;
  const legacy = patched.replace(/slug: "claude-sonnet-5",[\s\S]*?(?=\n\t\{\n\t\tslug:)/, (block) =>
    block
      .replace(
  ...
  const upgraded = patchBundle(legacy);
  const api = executePatchedFixture(upgraded.bundle);
  assert.equal(
    api
      .getClaudeModelCapabilities("claude-sonnet-5")
      .optionDescriptors.find((d) => d.id === "effort")
      .options.find((o) => o.isDefault).value,
    "medium",
  );
  assert.equal(api.resolveClaudeApiModelId({ model: "gpt-6-astra" }), "gpt-6-astra[effort=medium]");
  for (const model of [
    "gpt-6-astra",
    "claude-fable-5-1",
    "claude-opus-5",
    ...
    assert.equal(
      api
        ...
        .optionDescriptors.find((d) => d.id === "contextWindow")
        .options.some((o) => o.value === "900k"),
      true,
    );
    assert.equal(api.selectedClaudeContextWindow({ model }), 272000);
    assert.equal(api.buildAgentSettings({ model }).autoCompactWindow, 240000);
  ...
  assert.equal(patchBundle(upgraded.bundle).bundle, upgraded.bundle);
});
...
test("hybrid models compact at 240k by default and retain explicit extended context", () => {
  ...
  for (const model of [
    ...
    assert.deepEqual(
      runtime.buildTextSettings({ model, options: [] }),
      { autoCompactWindow: 240000 },
      model,
    );
    assert.deepEqual(
      runtime.buildAgentSettings({ model, options: [] }),
      { autoCompactWindow: 240000 },
      model,
    );
...
test("patch reserves Qwen output space by compacting its 131k window at 95k", () => {
  ...
  for (const models of [runtime.allModels, runtime.versionModels, runtime.pendingModels]) {
    ...
    const effort = qwen.capabilities.optionDescriptors.find(
    ...
    const context = qwen.capabilities.optionDescriptors.find(
  ...
  assert.equal(
    runtime.resolveClaudeApiModelId({
      model: "qwen3.8-27b",
  ...
  assert.deepEqual(runtime.buildTextSettings({ model: "qwen3.8-27b" }), {
    autoCompactWindow: 95000,
  });
  assert.deepEqual(runtime.buildAgentSettings({ model: "qwen3.8-27b" }), {
    autoCompactWindow: 95000,
  });
  assert.deepEqual(runtime.buildAgentSettings({ model: "claude-opus-5" }), {
    autoCompactWindow: 240000,
  });
});

test("Qwen gets a private 131k environment and switching to Sol applies 272k without mutating the base", () => {
  const runtime = executePatchedFixture();
  ...
  const qwenEnvironment = runtime.buildAgentEnvironment({ model: "qwen3.8-27b" }, baseEnvironment);
  ...
  assert.notEqual(solEnvironment, baseEnvironment);
  assert.equal(solEnvironment.CLAUDE_CODE_MAX_CONTEXT_TOKENS, "272000");
  assert.equal(solEnvironment.CLAUDE_CODE_AUTO_COMPACT_WINDOW, "240000");
  ...
  withAgentDirs(({ projectDir, homeDir }) => {
    ...
    assert.deepEqual(
      resolveSubagentDisplayConfig({
        ...
        projectDir,
        homeDir,
        sessionModel: "claude-sonnet-5[1m][effort=high]",
        sessionEffort: "high",
        launchInput: {},
      }),
...
test("unknown, unsafe, or malformed agent config falls back to the session", () => {
  withAgentDirs(({ projectDir, homeDir }) => {
    fs.writeFileSync(
      path.join(projectDir, ".claude", "agents", "broken.md"),
      "---\nmodel: haiku\neffort: xhigh\n",
      "utf8",
    );
    const fallback = { model: "claude-fable-5-1", effort: "medium" };
    for (const subagentType of ["missing", "../luna-worker", "broken"]) {
      assert.deepEqual(
        resolveSubagentDisplayConfig({
          ...
          projectDir,
          homeDir,
          sessionModel: fallback.model,
          sessionEffort: fallback.effort,
          launchInput: {},
        }),
        fallback,
      );
...
test("patches a copied archive without requiring its unpacked native sidecar", () => {
  ...
  try {
    ...
    const result = patchAsarTarget(
      { kind: "asar", archive: detached, entry },
